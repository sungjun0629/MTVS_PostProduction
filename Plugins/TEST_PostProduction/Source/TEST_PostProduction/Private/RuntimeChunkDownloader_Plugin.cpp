...
FRuntimeChunkDownloader_Plugin::FRuntimeChunkDownloader_Plugin() : bCanceled(false)
{

}
FRuntimeChunkDownloader_Plugin::~FRuntimeChunkDownloader_Plugin()
{
	//UE_LOG(LogRuntimeFilesDownloader, Warning, TEXT("FRuntimeChunkDownloader destroyed"));
}


TFuture<FRuntimeChunkDownloaderResult> FRuntimeChunkDownloader_Plugin::DownloadFile(const FString& URL, float Timeout, const FString& ContentType, int64 MaxChunkSize, const TFunction<void(int64, int64)>& OnProgress)
{
	if (bCanceled)
	{
		//UE_LOG(LogRuntimeFilesDownloader, Warning, TEXT("Canceled file download from %s"), *URL);
		return MakeFulfilledPromise<FRuntimeChunkDownloaderResult>(FRuntimeChunkDownloaderResult{ EDownloadToMemoryResult_Plugin::Cancelled, TArray64<uint8>() }).GetFuture();
	}

	TSharedPtr<TPromise<FRuntimeChunkDownloaderResult>> PromisePtr = MakeShared<TPromise<FRuntimeChunkDownloaderResult>>();
	TWeakPtr<FRuntimeChunkDownloader_Plugin> WeakThisPtr = AsShared();
	GetContentSize(URL, Timeout).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType, MaxChunkSize, OnProgress](int64 ContentSize) mutable
		{
			TSharedPtr<FRuntimeChunkDownloader_Plugin> SharedThis = WeakThisPtr.Pin();
			if (!SharedThis.IsValid())
			{
				//UE_LOG(LogRuntimeFilesDownloader, Warning, TEXT("Failed to download file chunk from %s: downloader has been destroyed"), *URL);
				PromisePtr->SetValue(FRuntimeChunkDownloaderResult{ EDownloadToMemoryResult_Plugin::DownloadFailed, TArray64<uint8>() });
				return;
			}
			...
			{
				//UE_LOG(LogRuntimeFilesDownloader, Warning, TEXT("Canceled file download from %s"), *URL);
				PromisePtr->SetValue(FRuntimeChunkDownloaderResult{ EDownloadToMemoryResult_Plugin::Cancelled, TArray64<uint8>() });
				return;
			}
			...
			auto OnChunkDownloaded = [WeakThisPtr, PromisePtr, URL, ContentSize, Timeout, ContentType, OnProgress, DownloadByPayload, OverallDownloadedDataPtr, bChunkDownloadedFilledPtr, ChunkOffsetPtr, OnChunkDownloadedFilled](TArray64<uint8>&& ResultData) mutable
				{
					...
					{
						PromisePtr->SetValue(FRuntimeChunkDownloaderResult{ EDownloadToMemoryResult_Plugin::Success, MoveTemp(*OverallDownloadedDataPtr.Get()) });
						OnChunkDownloadedFilled();
						return;
					}
			...
			SharedThis->DownloadFilePerChunk(URL, Timeout, ContentType, MaxChunkSize, ChunkRange, OnProgress, OnChunkDownloaded).Next([PromisePtr, bChunkDownloadedFilledPtr, URL, OverallDownloadedDataPtr, OnChunkDownloadedFilled, DownloadByPayload](EDownloadToMemoryResult_Plugin Result) mutable
				{
...
TFuture<EDownloadToMemoryResult_Plugin> FRuntimeChunkDownloader_Plugin::DownloadFilePerChunk(const FString& URL, float Timeout, const FString& ContentType, int64 MaxChunkSize, FInt64Vector2 ChunkRange, const TFunction<void(int64, int64)>& OnProgress, const TFunction<void(TArray64<uint8>&&)>& OnChunkDownloaded)
{
	if (bCanceled)
	{
		//UE_LOG(LogRuntimeFilesDownloader, Warning, TEXT("Canceled file chunk download from %s"), *URL);
		return MakeFulfilledPromise<EDownloadToMemoryResult_Plugin>(EDownloadToMemoryResult_Plugin::Cancelled).GetFuture();
	}

	TSharedPtr<TPromise<EDownloadToMemoryResult_Plugin>> PromisePtr = MakeShared<TPromise<EDownloadToMemoryResult_Plugin>>();
	TWeakPtr<FRuntimeChunkDownloader_Plugin> WeakThisPtr = AsShared();
	GetContentSize(URL, Timeout).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType, MaxChunkSize, OnProgress, OnChunkDownloaded, ChunkRange](int64 ContentSize) mutable
		{
			...
			{
				//UE_LOG(LogRuntimeFilesDownloader, Warning, TEXT("Failed to download file chunk from %s: downloader has been destroyed"), *URL);
				PromisePtr->SetValue(EDownloadToMemoryResult_Plugin::DownloadFailed);
				return;
			}
			...
			{
				//UE_LOG(LogRuntimeFilesDownloader, Warning, TEXT("Canceled file chunk download from %s"), *URL);
				PromisePtr->SetValue(EDownloadToMemoryResult_Plugin::Cancelled);
				return;
			}
				...
				SharedThis->DownloadFileByPayload(URL, Timeout, ContentType, OnProgress).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType, OnChunkDownloaded, OnProgress](FRuntimeChunkDownloaderResult Result) mutable
					{
						...
						if (Result.Result != EDownloadToMemoryResult_Plugin::Success && Result.Result != EDownloadToMemoryResult_Plugin::SucceededByPayload)
						{
			...
			SharedThis->DownloadFileByChunk(URL, Timeout, ContentType, ContentSize, ChunkRange, OnProgressInternal).Next([WeakThisPtr, PromisePtr, URL, Timeout, ContentType, ContentSize, MaxChunkSize, OnChunkDownloaded, OnProgress, ChunkRange](FRuntimeChunkDownloaderResult&& Result)
				{
					...
					if (ContentSize > ChunkRange.Y + 1)
					{
						...
						SharedThis->DownloadFilePerChunk(URL, Timeout, ContentType, MaxChunkSize, FInt64Vector2(ChunkStart, ChunkEnd), OnProgress, OnChunkDownloaded).Next([WeakThisPtr, PromisePtr](EDownloadToMemoryResult_Plugin Result)
							{
					...
					{
						PromisePtr->SetValue(EDownloadToMemoryResult_Plugin::Success);
					}
				});
		});
...
TFuture<FRuntimeChunkDownloaderResult> FRuntimeChunkDownloader_Plugin::DownloadFileByChunk(const FString& URL, float Timeout, const FString& ContentType, int64 ContentSize, FInt64Vector2 ChunkRange, const TFunction<void(int64, int64)>& OnProgress)
{
	if (bCanceled)
	{
		//UE_LOG(LogRuntimeFilesDownloader, Warning, TEXT("Canceled file download from %s"), *URL);
	...
	if (ChunkRange.X < 0 || ChunkRange.Y <= 0 || ChunkRange.X > ChunkRange.Y)
	{
		...
		return MakeFulfilledPromise<FRuntimeChunkDownloaderResult>(FRuntimeChunkDownloaderResult{ EDownloadToMemoryResult_Plugin::DownloadFailed, TArray64<uint8>() }).GetFuture();
	}
	...
	HttpRequestRef->OnProcessRequestComplete().BindLambda([WeakThisPtr, PromisePtr, URL, ChunkRange](FHttpRequestPtr Request, FHttpResponsePtr Response, bool bSuccess) mutable
		{
			...
			PromisePtr->SetValue(FRuntimeChunkDownloaderResult{ EDownloadToMemoryResult_Plugin::Success, TArray64<uint8>(Response->GetContent()) });
		});
...
TFuture<FRuntimeChunkDownloaderResult> FRuntimeChunkDownloader_Plugin::DownloadFileByPayload(const FString& URL, float Timeout, const FString& ContentType, const TFunction<void(int64, int64)>& OnProgress)
{
	if (bCanceled)
	{
		//UE_LOG(LogRuntimeFilesDownloader, Warning, TEXT("Canceled file download from %s"), *URL);
	...
	HttpRequestRef->OnProcessRequestComplete().BindLambda([WeakThisPtr, PromisePtr, URL](FHttpRequestPtr Request, FHttpResponsePtr Response, bool bSuccess) mutable
		{
			...
			return PromisePtr->SetValue(FRuntimeChunkDownloaderResult{ EDownloadToMemoryResult_Plugin::SucceededByPayload, TArray64<uint8>(Response->GetContent()) });
		});
...
TFuture<int64> FRuntimeChunkDownloader_Plugin::GetContentSize(const FString& URL, float Timeout)
{
...
void FRuntimeChunkDownloader_Plugin::CancelDownload()
{
	bCanceled = true;
