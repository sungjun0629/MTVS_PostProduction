 ...
 */
using FRuntimeChunkDownloaderResult = struct { EDownloadToMemoryResult_Plugin Result; TArray64<uint8> Data; };
...
class FRuntimeChunkDownloader_Plugin : public TSharedFromThis<FRuntimeChunkDownloader_Plugin>
{
public:
	FRuntimeChunkDownloader_Plugin();
	virtual ~FRuntimeChunkDownloader_Plugin();
	 ...
	 */
	virtual TFuture<EDownloadToMemoryResult_Plugin> DownloadFilePerChunk(const FString& URL, float Timeout, const FString& ContentType, int64 MaxChunkSize, FInt64Vector2 ChunkRange, const TFunction<void(int64, int64)>& OnProgress, const TFunction<void(TArray64<uint8>&&)>& OnChunkDownloaded);
